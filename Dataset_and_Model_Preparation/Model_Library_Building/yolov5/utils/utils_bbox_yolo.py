...
class DecodeBox_yolo():
    def __init__(self, anchors, num_classes, input_shape, anchors_mask = [[6,7,8], [3,4,5], [0,1,2]]):
        super(DecodeBox_yolo, self).__init__()
    ...
    def non_max_suppression_ori(self, prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5,
                            nms_thres=0.4):
        #   将预测结果的格式转换成左上角右下角的格式。
        #   prediction  [batch_size, num_anchors, 85]
        box_corner = prediction.new(prediction.shape)
        ...
        for i, image_pred in enumerate(prediction):
            ...
            if output[i] is not None:
                ...
                box_xy, box_wh = (output[i][:, 0:2] + output[i][:, 2:4]) / 2, output[i][:, 2:4] - output[i][:, 0:2]
                output[i][:, :4] = self.yolo_correct_boxes(box_xy, box_wh, input_shape, image_shape, letterbox_image)
                boxes_real = self.yolo_boxes2tensor(1001,input_shape, image_shape, letterbox_image)
                # self.m_bbox(prediction[0], boxes_real, class_conf, 0.5, 10)
                self.m_bbox(detections, boxes_real, class_conf, 0.5, 10)
        return output
