...
class YOLOLoss(nn.Module):
    ...
    def forward(self, l, input, targets=None, y_true=None):
        ...
        if n != 0:
            ...
            loss        += loss_loc * self.box_ratio + loss_cls * self.cls_ratio #zz

            # loss += loss_loc # 只取loss_loc
            #-----------------------------------------------------------#
            #   计算置信度的loss
        ...
        loss        += loss_conf * self.balance[l] * self.obj_ratio #zz
        # if n != 0:
...
def is_parallel(model):
    # Returns True if nets is of type DP or DDP
    return type(model) in (nn.parallel.DataParallel, nn.parallel.DistributedDataParallel)

def de_parallel(model):
    # De-parallelize a nets: returns single-GPU nets if nets is of type DP or DDP
...
class ModelEMA:
    ...
    def __init__(self, model, decay=0.9999, tau=2000, updates=0):
        ...
        # if next(nets.parameters()).device.type != 'cpu':
    ...
    def update(self, model):
        ...
        with torch.no_grad():
            ...
            msd = de_parallel(model).state_dict()  # nets state_dict
