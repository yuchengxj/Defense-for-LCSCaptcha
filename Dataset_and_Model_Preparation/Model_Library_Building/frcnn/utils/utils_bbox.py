...
class DecodeBox():
    ...
    def frcnn_correct_boxes(self, box_xy, box_wh, input_shape, image_shape, letterbox_image):
        #-----------------------------------------------------------------#
    ...
    def forward(self, roi_cls_locs, roi_scores, rois, image_shape, input_shape, letterbox_image, nms_iou = 0.3, confidence = 0.5):
        results = []
        bs      = len(roi_cls_locs)
        #--------------------------------#
        ...
        for i in range(bs):
            ...
            if len(results[-1]) > 0:
                ...
                results[-1][:, :4] = self.frcnn_correct_boxes(box_xy, box_wh, input_shape, image_shape, letterbox_image)
