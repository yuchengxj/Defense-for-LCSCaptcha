...
class MultiboxLoss(nn.Module):
    ...
    def forward(self, y_true, y_pred):
        ...
        # --------------------------------------------- #
        conf_loss = self._softmax_loss(y_true[:, :, 4:], y_pred[:, :, 4:])
        ...
        #total_loss  = torch.sum(pos_conf_loss) + torch.sum(self.alpha * pos_loc_loss)
        total_loss  = total_loss / torch.sum(num_pos)
        return total_loss
        # return torch.sum(self.alpha * pos_loc_loss)/torch.sum(num_pos) ## 只取locloss
